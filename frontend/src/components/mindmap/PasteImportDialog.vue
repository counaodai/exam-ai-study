...
import { parseQuestions, importParsedQuestions } from '@/api/questionImport'
...
async function handleParse() {
  ...
  try {
    ...
    if (result.questions.length === 0) {
      ElMessage.warning('未能识别到有效的题目结构，请检查格式后重试')
    } else {
      initEditedQuestions()
      step.value = 2
      if (result.parse_errors.length > 0) {
        ElMessage.warning(`识别到 ${result.questions.length} 道题目，${result.parse_errors.length} 道解析不完整`)
      }
    }
  } catch (err: any) {
...
async function handleConfirmImport() {
  ...
  try {
    const result = await importParsedQuestions({
...
<template>
  <el-dialog
    ...
    :title="step === 1 ? '粘贴题目内容' : step === 2 ? '识别结果预览' : '导入结果'"
    width="700px"
  ...
  >
    ...
    <div v-if="step === 1" class="paste-step">
      <el-input
        ...
        type="textarea"
        :rows="12"
        placeholder="请粘贴题目内容，支持纯文本和Markdown格式&#10;&#10;示例：&#10;1. 某公司计划在三个城市...&#10;A. 方案一  B. 方案二&#10;C. 方案三  D. 方案四&#10;答案：B&#10;解析：本题考查排列组合..."
        :class="{ 'is-error': pasteContent.length > 10000 }"
      />
      <div class="paste-hint">
        <span>支持格式：纯文本、Markdown</span>
        <span v-if="pasteContent.length > 10000" class="error-text">内容过长，建议分批导入</span>
        <span v-else>多题请用题号分隔</span>
      </div>
    </div>
    ...
    <div v-if="step === 2" class="preview-step">
      <div class="preview-header">
        <span>识别到 <strong>{{ parsedQuestions.length }}</strong> 道题目</span>
        <span v-if="parseErrors.length > 0" class="warning-text">
          （{{ parseErrors.length }} 道解析不完整）
        </span>
      </div>
          ...
          <el-form label-width="60px" size="small">
            ...
            <el-form-item label="选项">
              <div class="options-editor">
                ...
                <el-button size="small" @click="addOption(qi)">+ 添加选项</el-button>
              </div>
            </el-form-item>

            <el-form-item label="答案">
              <el-input v-model="q.answer" placeholder="如：B" style="width: 120px" />
            </el-form-item>
